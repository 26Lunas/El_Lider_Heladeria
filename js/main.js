document.addEventListener("DOMContentLoaded", function () {
    ...
    .then(() => {
        initModalLogic();
        initCategoryFilter(); // Initialize category filter after components are loaded
    })
    .catch(error => console.error("Error al inicializar la página:", error));

    // Function to initialize category filter
    function initCategoryFilter() {
        const categoryButtons = document.querySelectorAll('.d-flex.justify-content-center.mb-4 .btn');
        const productCards = document.querySelectorAll('.cont-card-product .col-12');
